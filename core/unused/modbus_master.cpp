...
using namespace std;
#define MAX_LOCAL_INPUT		1
#define MAX_LOCAL_OUTPUT	1
#define MAX_ANALOG_OUT		0

int inBufferPinMask[MAX_LOCAL_INPUT]  =       { 142 };
int outBufferPinMask[MAX_LOCAL_OUTPUT]=       { 144 };
...
void initializeHardware()
{
	...
	for (int i = 0; i < num_devices; i++)
	{
		...
		{
			mb_devices[i].mb_ctx = modbus_new_rtu(	mb_devices[i].dev_address, mb_devices[i].rtu_baud, mb_devices[i].rtu_parity, mb_devices[i].rtu_data_bit, mb_devices[i].rtu_stop_bit);
		}
